...
BEGIN TRY
	...
	OR @TableName = 'DisasterTracking' AND OBJECT_ID('DataMart_History.DisasterTracking' ) IS NOT NULL
		...
		CREATE CLUSTERED INDEX IX_DataMart_History_DisasterTracking_CI ON DataMart_History.DisasterTracking(RecordEndDate, LoanNumber, DisasterTrackingId) WITH (DATA_COMPRESSION = PAGE) ON [PRIMARY]; -- ps_DateByMonthRight (RecordEndDate)
		END;
END TRY
BEGIN CATCH
	SELECT * FROM Processing.vw_ReturnedError;
