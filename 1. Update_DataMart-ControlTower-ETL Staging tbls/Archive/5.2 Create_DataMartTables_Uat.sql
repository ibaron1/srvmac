CREATE PROCEDURE [Processing].[Create_DataMartTables_Uat] @TableName VARCHAR(100) = NULL
AS
/* All data elements */
...
WHEN S.DataType = 'decimal' THEN
	CONCAT(S.DataType, '(', S.DecimalPrecision, ',', S.DecimalScale, ')')
ELSE
	S.DataType
END AS DataType
, S.PrimaryKeyIndex
...
SELECT @DropTableSQL = STRING_AGG(CONCAT('DROP TABLE ', T.TABLE_SCHEMA, '.', T.TABLE_NAME), ' ; ')
...
WHILE @CreateTableSeq <= (SELECT MAX(Seq) FROM #CreateTableSq1)
BEGIN
	SELECT @CreateTableSQL = CTS.CreateTableSQL
	FROM #CreateTableSq1 AS CTS
